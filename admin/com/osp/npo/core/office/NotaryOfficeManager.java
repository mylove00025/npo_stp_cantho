 ...
 * @version $Revision: 29717 $ 
 */
public class NotaryOfficeManager extends AbstractManager {

	private Long nidFilter;
    private String authenticationIdFilter;
    private Byte officeTypeFilter;
    private String notaryNameFilter;
    private Boolean ActFlag;
    private Boolean HidFlag;

	/**
	 * @return the nidFilter
	 */
	public Long getNidFilter() {
		return nidFilter;
	}


	/**
	 * @param nidFilter the nidFilter to set
	 */
	public void setNidFilter(Long nidFilter) {
		this.nidFilter = nidFilter;
	}
    ...
    public void setAuthenticationIdFilter(String authenticationIdFilter, FilterKind kind) {
        ...
        this.authenticationIdFilter = this.buildFilterString(authenticationIdFilter, kind.getValue());
    }
    
       
    /**
	 * Set officeTypeFilter
	 * @param officeTypeFilter the officeTypeFilter to set
	 */
	public void setOfficeTypeFilter(Byte officeTypeFilter) {
		this.officeTypeFilter = officeTypeFilter;
	}
	
	 /**
	 * Set ActiveFlagFilter
	 * @param activeFlagFilter the activeFlagFilter to set
	 */
	public void setActFlagFilter(Boolean actFlagFilter) {
		this.ActFlag = actFlagFilter;
	}
	
	 /**
	 * Set HiddenFlagFilter
	 * @param hiddenFlagFilter the hiddenFlagFilter to set
	 */
	public void setHidFlagFilter(Boolean hidFlagFilter) {
		this.HidFlag = hidFlagFilter;
	}
    ...
    public int insert(NotaryOfficeInfo info) throws SQLException {
        ...
        try {
            ...
            int i = 1;
            psSetByte(ps, i++, info.getOfficeType());
     ...
     * @throws IOException
     */
    public NotaryOfficeList select(boolean forUpdate, int offset, int limit) throws SQLException, IOException {
    ...
    private void makeUpdateList(NotaryOfficeInfo info, ArrayList<String> sqlList,ArrayList<Object> prmList){
    	...
    	addUpdateList(info.getOfficeType(), ",office_type=?", sqlList, prmList);
    ...
    private String getWhereFilter(ArrayList<Object> prmList) {
        ...
        if (this.nidFilter != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("noid = ?");
            prmList.add(this.nidFilter);
        }
        
        if (this.authenticationIdFilter != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("authentication_id LIKE ?");
            prmList.add(this.authenticationIdFilter);
        }
        
        if (this.officeTypeFilter != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("office_type = ?");
            prmList.add(this.officeTypeFilter);
        }
        
        if (this.notaryNameFilter != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("name LIKE ?");
            prmList.add(this.notaryNameFilter);
        }
        
        if (this.ActFlag != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("active_flg = ?");
            prmList.add(this.ActFlag);
        }
        
        if (this.HidFlag != null) {
            appendAnd(filterBuffer);
            filterBuffer.append("hidden_flg = ?");
            prmList.add(this.HidFlag);
        }
        
        return filterBuffer.toString();
    }

    public void setNotaryNameFilter(String notaryNameFilter, FilterKind kind) {
		this.notaryNameFilter = this.buildFilterString(notaryNameFilter, kind.getValue());
	}
